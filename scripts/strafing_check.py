"""CS2 急停指标验证：击杀瞬间的移动速度分布（demo 分析 MVP 核心逻辑）"""
import sys
import numpy as np
import pandas as pd
...
def main():
    ...
    # 4) 对每次击杀，取击杀者开枪前 1 tick 的速度（急停判定）
    #    用 attacker 去 tick 表查速度；attacker 不在表里的（如爆头自杀/环境）跳过
    rows = []
    for _, k in kills.iterrows():
        atk = k["attacker_steamid"]
        t = k["tick"]
        # 事件 steamid 是 str，tick 表是 uint64，统一转 int 再匹配
        try:
            atk = int(atk)
